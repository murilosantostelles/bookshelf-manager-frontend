...
const CHAVE_DEVOLVIDOS = 'emprestimosDevolvidos';

const Livros = () => {
  const [livros, setLivros] = useState<Livro[]>([]);
  const [emprestimos, setEmprestimos] = useState<Emprestimo[]>([]);
  const [devolvidos, setDevolvidos] = useState<number[]>([]);
  ...
  const carregarDados = async () => {
    try {
      const [livrosResponse, emprestimosResponse] = await Promise.all([
        api.get('/livros?page=0&size=100'),
        api.get('/emprestimos'),
      ]);

      setLivros(livrosResponse.data.content ?? []);
      setEmprestimos(emprestimosResponse.data ?? []);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const devolvidosSalvos = localStorage.getItem(
      CHAVE_DEVOLVIDOS
    );

    if (devolvidosSalvos) {
      try {
        const ids = JSON.parse(devolvidosSalvos);
        ...
        console.error(
          'Erro ao carregar empréstimos devolvidos:',
          error
        );
      }
    }
  ...
  const obterStatusLivro = (livro: Livro) => {
    const tituloLivro = normalizarTitulo(livro.titulo);

    const possuiEmprestimoAtivo = emprestimos.some(
      (emprestimo) => {
        const mesmoLivro =
          normalizarTitulo(emprestimo.livroTitulo) ===
          tituloLivro;

        const foiDevolvido =
          devolvidos.includes(emprestimo.id);

        return mesmoLivro && !foiDevolvido;
      }
    );
  ...
  const livrosComStatus = useMemo(() => {
    return livros.map((livro) => ({
      ...livro,
      statusAtual: obterStatusLivro(livro),
    }));
  }, [livros, emprestimos, devolvidos]);

  const livrosFiltrados = livrosComStatus.filter((livro) => {
    ...
    const matchStatus =
      statusFiltro === 'TODOS' ||
      livro.statusAtual === statusFiltro;
  ...
  const statusLivroSelecionado = livroSelecionado
    ? obterStatusLivro(livroSelecionado)
    : 'DISPONIVEL';
              ...
              >
                ...
                <div className="p-3 h-28 flex flex-col">
                  ...
                  <div className="mt-auto">
                    <span
                      className={`inline-block text-xs px-2 py-0.5 rounded-full font-medium ${obterClasseStatus(
                        livro.statusAtual
                      )}`}
                    >
                      {obterTextoStatus(livro.statusAtual)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="flex-shrink-0 flex justify-center">
                    <div className="w-40 h-56 bg-stone-100 rounded-lg overflow-hidden flex items-center justify-center">
                ...
                {/* Palavras-chave */}
                {livroSelecionado.palavrasChave?.length > 0 && (
                  <div className="mt-6">
                    <h4 className="text-sm font-semibold text-stone-800 mb-2">
                      Palavras-chave
                    </h4>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-stone-600 mt-2">
                    Deseja realmente excluir o livro da sua
                    biblioteca?
                  </p>
